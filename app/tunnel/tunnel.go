	...
	Subcommands: []*cli.Command{
		{
			...
			Action: func(c *cli.Context) error {
				...
				return createTCPTunnel(c.Context, client, namespace, name, port)
			},
		},
	},
}
...
func createTCPTunnel(ctx context.Context, client kubernetes.Client, namespace, name, port string) error {
	if name == "" {
	...
	service := &corev1.Service{
		...
		Spec: corev1.ServiceSpec{
			...
			Ports: []corev1.ServicePort{
				{
					Name: fmt.Sprintf("tcp-%d", remotePort),

					Protocol: corev1.ProtocolTCP,
					Port:     int32(remotePort),

					TargetPort: intstr.FromInt(remotePort),
				},
			},
		},
	}
					...
					Ports: []corev1.ContainerPort{
						{
							...
							Protocol:      corev1.ProtocolTCP,
							ContainerPort: int32(remotePort),
						},
					},
				},
			},
	...
	return tunnelTCP(ctx, client, namespace, name, port)
}

func tunnelTCP(ctx context.Context, client kubernetes.Client, namespace, name, port string) error {
	service, err := client.WaitForService(ctx, namespace, name)
	...
	cli.Infof("Forwarding tcp://%s:%s => http://localhost:%s", address, port, port)
